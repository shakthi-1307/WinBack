...
SQLite rather than Postgres on purpose: the whole system has to run with no
external services, and nothing here needs concurrent writers. The schema
...
class Ledger:
    ...
    def append(self, day: int, txn_id: str, type_: str, **payload: Any) -> None:
        self._conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            (self.run_id, self.strategy, day, txn_id, type_,
    ...
    def events_for(self, txn_id: str) -> list[dict]:
        ...
        return [{"seq": r[0], "day": r[1], "type": r[2], **json.loads(r[3])}
                for r in rows]

    def counts_by_type(self) -> dict[str, int]:
        rows = self._conn.execute(
            "SELECT type, COUNT(*) FROM events WHERE run_id = ? "
    ...
    def recent(self, type_: str, limit: int = 50) -> list[dict]:
        rows = self._conn.execute(
            "SELECT seq, day, txn_id, payload FROM events "
            "WHERE run_id = ? AND type = ? ORDER BY seq DESC LIMIT ?",
            (self.run_id, type_, limit),
        ).fetchall()
        return [{"seq": r[0], "day": r[1], "txn_id": r[2], **json.loads(r[3])}
                for r in rows]
